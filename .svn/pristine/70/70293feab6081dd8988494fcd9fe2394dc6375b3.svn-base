...
function send_mail($name, $email, $subject, $content, $type = 0, $notification=false)
{
    ...
    else
    {
        ...
        $params['ssl'] = $GLOBALS['_CFG']['smtp_ssl'];
        ...
        else
        {
            ...
            //edit by wuzeguo
            include_once(ROOT_PATH . 'includes/emailer.php');
            static $smtp;
            if (!isset($smtp))
            {
                logger::write('params:' . json_encode($params));
                $params['smtp_ssl'] && $params['host'] = 'ssl://' . $params['host'];

                $smtp = new smtp($params['host'], $params['port'], 'true', $params['user'], $params['pass']);
            }
            $smtp->debug = false;
            //测试发送
            if (!$smtp->smtp_sockopen()) {
                logger::write('email send error');
                $GLOBALS['err']->add('sockopen error');
            }
                logger::write('email :' . $email);
                logger::write('shop name :' . $shop_name);
                logger::write('subj:' . $subject);
                logger::write('Content :' . $content);
            $type = $type == 1 ? 'HTML' : 'TXT';
            $result = $smtp->sendmail($email, $shop_name, '', $subject, $content, $type);
            if ($result) {
                return true;
            } else {
...
function trace($arg, $file = '', $line = '')
{
    ...
    $destination = ROOT_PATH."logs/".date('Y-m-d')."_trace.log";
...
function log_write_back($arg, $file = '', $line = '')
{
    if ((DEBUG_MODE & 4) != 4)
    {
        //return;
 ...
 * @return array 
 */
function syn_data_to_licai($data, $func)
{
    require_once (dirname(__FILE__) . '/cls_syn_user.php');
