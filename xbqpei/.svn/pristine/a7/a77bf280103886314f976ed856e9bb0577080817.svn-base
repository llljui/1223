...
class cls_mysql
{
    ...
    {
        DEBUG_MODE && $this->max_cache_time = 300;
...
/* 仿真 Adodb 函数 */
    ...
    function add($arr) 
    {
        ...
        //添加成功返回成功的id
        if ($this->autoExecute($this->_table, $arr, 'INSERT', $this->_where))
        {
            $this->cleanSql();
    ...
    function edit($arr)
    {
        ...
        $result = $this->autoExecute($this->_table, $arr, 'UPDATE', $this->_where);
        $this->cleanSql();
        return $result;
    }
    function select()
    {
        $sql = $this->parseSelectSql();
        $this->cleanSql();
        return $this->getAll($sql);
    }
    function find()
    ...
    function getField($field, $col = false)
    {
        $this->_field = $field;
        $sql = $this->parseSelectSql();
        $this->cleanSql();
    ...
    function limit($start = 0, $end = null)
    {
        if ($start && $end) {
    ...
    function cleanSql()
    {
        $this->_table = $this->_where = $this->_limit = $this->_order = '';
        $this->_field = '*';
    }
    /**end ***/
