...
elseif ($_REQUEST['act'] == 'insert')
{
	...
	$other['office_phone'] = isset($_POST['mobile_phone']) ? htmlspecialchars(trim($_POST['mobile_phone'])) : '';
	$other['home_phone'] = isset($_POST['mobile_phone']) ? htmlspecialchars(trim($_POST['mobile_phone'])) : '';
...

{
    ...
    /*edit by wuzeguo*/
    if ($user['face_card']) {
        if (strpos(strtolower($user['face_card']), 'http') === false) {
            $user['face_card'] = '../' . $user['face_card'];
        }
    }
    if ($user['back_card']) {
        if (strpos(strtolower($user['back_card']), 'http') === false) {
            $user['back_card'] = '../' . $user['back_card'];
        }
    }
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    $user_info1 = $GLOBALS['db']->table('users')->where('user_id = ' . $user['user_id'])->field('back_card, face_card')->find();
    $syn_data = array(
        'user_name' => $username,
        'email'    => $email,
        'mobile'   => $mobile_phone,
        'sex'      => $sex,
        'real_name' => $real_name,
        'back_card' => 'http://' . $host . '/' .$user_info1['back_card'],
        'face_card' => 'http://' . $host . '/' .$user_info1['face_card'],
    );
	...
	if($face_card != '')
	{
        $syn_data['face_card'] = 'http://' . $host . '/' .$face_card;
		 $sql = "update ".$ecs->table('users')." set `face_card` = '$face_card' where user_name = '".$username."'";
		 $db->query($sql);
	}
	if($back_card != '')
	{
        $syn_data['back_card'] = 'http://' . $host . '/' .$back_card;
		 $sql = "update ".$ecs->table('users')." set `back_card` = '$back_card' where user_name = '".$username."'";
		 $db->query($sql);
	}
	...
	**/
    ...
    //同步信息
    if ($status == 1) {
        syn_data_to_licai($syn_data, 'syn_real_info');
    }
