...
function action_act_edit_profile ()
{
    ...
    $result_obj = $syn_obj->syn_edit_user($data);
    if (!$result_obj->response_code) {
		show_message('用户名可能已被占用，请重新填写', '', '', 'info');
        die;
    }
...
function action_act_identity ()
{
	...
	if($num > 0)
	{
        ...
        $data = array(
            'real_name' => $real_name,
            'idno' => $card,
            'face_card' => 'http://' . $host . '/' .$rows['face_card'],
            'back_card' => 'http://' . $host . '/' . $rows['back_card']
        );
        syn_data_to_licai($data, 'syn_submit_real_info');
