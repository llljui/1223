...
function assign_ur_here($cat = 0, $str = '')
{
    ...
    {
        /* 处理有分类的 */
        if (in_array($filename, array('car_system_category','category', 'goods', 'article_cat', 'article', 'brand')))
        {
            /* 商品分类或商品 */
            ...
            elseif ('car_system_category' == $filename) {
            
                if ($cat > 0)
                {
                    $cat_arr = get_car_system_parent_cats($cat);

                    $key     = 'cid';
                    $type    = 'car_system_category';
                }
                else
                {
                    $cat_arr = array();
...
function assign_pager($app, $cat, $record_count, $size, $sort, $order, $page = 1,
                        $keywords = '', $brand = 0, $price_min = 0, $price_max = 0, $display_type = 'list', $filter_attr='', $url_format='', $sch_array='',$filter=0, $output=0, $year=0)
{
    ...
    switch ($app)
    {
    	case 'supplier':
            ...
            break;
        case 'category':
            $uri_args = array('cid' => $cat, 'bid' => $brand, 'price_min'=>$price_min, 'price_max'=>$price_max,'filter'=>$filter, 'year' => $year, 'output' => $output, 'filter_attr'=>$filter_attr, 'sort' => $sort, 'order' => $order, 'display' => $display_type);
            break;
    ...
    else
    {
        ...
        {
            $pager['click_page_first'] = build_uri($app, $uri_args, '', 1, $keywords);
            $pager['click_page_last'] = build_uri($app, $uri_args, '', $page_count, $keywords);
    ...
    $GLOBALS['smarty']->assign('shangjian', build_uri($app, array('cid'=>$cat,'display'=>$pager['display'],'bid'=>$brand,'price_min'=>$price_min,'price_max'=>$price_max,'filter'=>$filter,'year' =>$year, 'output' => $output,'filter_attr'=>$filter_attr,'page'=>$pager['page'],'sort'=>'goods_id','order'=>(($pager['sort'] == 'goods_id' && $pager['order'] == 'DESC') ? 'ASC' : 'DESC'))));
    $GLOBALS['smarty']->assign('xiaoliang', build_uri($app, array('cid'=>$cat,'display'=>$pager['display'],'bid'=>$brand,'price_min'=>$price_min,'price_max'=>$price_max,'filter'=>$filter,'year' =>$year, 'output' => $output,'filter_attr'=>$filter_attr,'page'=>$pager['page'],'sort'=>'salenum','order'=>(($pager['sort'] == 'salenum' && $pager['order'] == 'DESC') ? 'ASC' : 'DESC'))));
    $GLOBALS['smarty']->assign('jiage', build_uri($app, array('cid'=>$cat,'display'=>$pager['display'],'bid'=>$brand,'price_min'=>$price_min,'price_max'=>$price_max,'filter'=>$filter,'year' =>$year, 'output' => $output,'filter_attr'=>$filter_attr,'page'=>$pager['page'],'sort'=>'shop_price','order'=>(($pager['sort'] == 'shop_price' && $pager['order'] == 'DESC') ? 'ASC' : 'DESC'))));
    $GLOBALS['smarty']->assign('gengxin', build_uri($app, array('cid'=>$cat,'display'=>$pager['display'],'bid'=>$brand,'price_min'=>$price_min,'price_max'=>$price_max,'filter'=>$filter,'year' =>$year, 'output' => $output,'filter_attr'=>$filter_attr,'page'=>$pager['page'],'sort'=>'last_update','order'=>(($pager['sort'] == 'last_update' && $pager['order'] == 'DESC') ? 'ASC' : 'DESC'))));
    $GLOBALS['smarty']->assign('renqi', build_uri($app, array('cid'=>$cat,'display'=>$pager['display'],'bid'=>$brand,'price_min'=>$price_min,'price_max'=>$price_max,'filter'=>$filter,'year' =>$year, 'output' => $output,'filter_attr'=>$filter_attr,'page'=>$pager['page'],'sort'=>'click_count','order'=>(($pager['sort'] == 'click_count' && $pager['order'] == 'DESC') ? 'ASC' : 'DESC'))));
...
function get_pager($url, $param, $record_count, $page = 1, $size = 10)
{
    ...
    else
    {
        ...
        $pager['click_page_first'] = $url_format . 1;
        $pager['click_page_last'] = $url_format . $page_count;
        $pager['page_first'] = ($page - $_offset > 1 && $_pagenum < $page_count) ? $url_format . 1 : '';
        $pager['page_prev']  = ($page > 1) ? $url_format . $page_prev : '';
        $pager['page_next']  = ($page < $page_count) ? $url_format . $page_next : '';
        $pager['page_last']  = ($_to < $page_count) ? $url_format . $page_count : '';
