...
elseif ($_REQUEST['act']== 'add')
{
    ...
    //所有用户没开过商铺
    $user_ids = $db->table('supplier')->getField('user_id', true);
    $user_ids = implode(',', $user_ids);
    $user_list = $db->table('users')->field('user_id, user_name')->where('user_id NOT IN (' . $user_ids . ') AND status = 1')->select();
    $smarty->assign('user_list', $user_list);

    $smarty->assign('ur_here', $_LANG['add_supplier']);
    $lang_supplier_list = $_LANG['02_supplier_list'];
    $smarty->assign('action_link', array('href' => 'supplier.php?act=list&status=1', 'text' =>$lang_supplier_list ));
    ...
    $smarty->display('add_supplier.htm');
...
elseif ($_REQUEST['act']=='insert')
{
   ...
   $supplier = array(
                            ...
                            'add_time' => time(),
                            'applynum' =>3,
                            'contacts_name' => $user_info['user_name'],
                            'contacts_phone' => $user_info['mobile_phone'],
                            ...
                            'id_card_no' => $user_info['card'],
							
                           );

   //入库
   $supplier_id = $db->table('supplier')->add($supplier);

   //密码:123456 + $user_info['ec_salt'];
   if ($user_info['ec_salt']) {
       $pass = md5(md5('123456') . $user_info['ec_salt']);
   } else {
       $pass = md5('123456');
   }

   //添加账户supplier_admin_user
   $arr = array(
   ...
   $db->table('supplier_admin_user')->add($arr);

   /* 清除缓存 */
   clear_cache_files();

   /* 提示信息 */
   $links[] = array('href' => 'supplier.php?act=list&status=1', 'text' => $_LANG['back_supplier_list']);
	sys_msg($_LANG['add_supplier_ok'], 0, $links);    
