...
//找出父类名称
if ($index_file['parent_id']) {
    $parent_info = $db->table('car_system_category')->where('cat_id = ' . $index_file['parent_id'])->find();
    if ($parent_info['parent_id']) {
        $parent_name = $db->table('car_system_category')->where('cat_id = ' . $parent_info['parent_id'])->getField('cat_name');
    } else {
        $parent_name = $parent_info['cat_name'];
    }
} else {
    $parent_name = $index_file['cat_name'];
}
...
else
{
	...
	$output = (isset($_REQUEST['output'])) ? intval($_REQUEST['output']) : 0;
	...
	$cache_id = sprintf('%X', crc32($cat_id . '-' . $display . '-' . $sort  .'-' . $order  .'-' . $page . '-' . $size . '-' . $_SESSION['user_rank'] . '-' .
	    $_CFG['lang'] .'-'. $brand. '-' . $price_max . '-' .$price_min . '-' . $filter_attr_str . '-' . $filter . '-' . $year . '-' . $output));
	...
	if (!$smarty->is_cached($index_dwt_files, $cache_id))
	{
		...
		if ($cat['grade'] > 1)
		{
			...
			*/
			...
			foreach ($price_grade as $key=>$val)
			{
				...
				$price_grade[$temp_key]['url'] = build_uri('car_system_category', array('cid'=>$cat_id, 'bid'=>$brand, 'price_min'=>$price_grade[$temp_key]['start'], 'price_max'=> $price_grade[$temp_key]['end'], 'filter_attr'=>$filter_attr_str, 'filter'=>$filter, 'year' => $year, 'output' => $output), $cat['cat_name']);
			...
			$price_grade[0]['url'] = build_uri('car_system_category', array('cid'=>$cat_id, 'bid'=>$brand, 'price_min'=>0, 'price_max'=> 0, 'filter_attr'=>$filter_attr_str, 'filter'=>$filter, 'year' => $year, 'output' => $output), $cat['cat_name']);
		...
		if ($brand)
		{
				$condition[] = array(
					...
					'cond_url' => build_uri('car_system_category', array('cid' => $cat_id, 'bid' => 0, 'price_min'=>$price_min, 'price_max'=> $price_max, 'filter_attr'=>$filter_attr_str, 'filter'=>$filter, 'year' => $year, 'output' => $output), $cat['cat_name']) ,
				);
		}
		...
		if ($price_min || $price_max)
		{
				$condition[] =array(
						...
						'cond_url' => build_uri('car_system_category', array('cid'=>$cat_id, 'bid'=>$brand, 'price_min'=>0, 'price_max'=> 0, 'filter_attr'=>$filter_attr_str, 'filter'=>$filter, 'year' => $year, 'output' => $output), $cat['cat_name'])
					); 
		}
		$smarty->assign('brand_zimu_68ecshop', $brand_zimu);
		$smarty->assign('url_no_price', build_uri('car_system_category', array('cid'=>$cat_id, 'bid'=>$brand,  'filter_attr'=>$filter_attr_str, 'filter'=>$filter, 'year' => $year, 'output' => $output), $cat['cat_name']));

		$brands[0]['brand_name'] = $_LANG['all_attribute'];
		$brands[0]['url'] = build_uri('car_system_category', array('cid' => $cat_id, 'bid' => 0, 'price_min'=>$price_min, 'price_max'=> $price_max, 'filter_attr'=>$filter_attr_str, 'filter'=>$filter, 'year' => $year, 'output' => $output), $cat['cat_name']);
		...
		if ($cat['filter_attr'] > 0)
		{
			...
			foreach ($cat_filter_attr AS $key => $value)
			{
				...
				if($temp_name = $db->getOne($sql))
				{
					...
					$all_attr_list[$key]['attr_list'][0]['url'] = build_uri('car_system_category', array('cid'=>$cat_id, 'bid'=>$brand, 'price_min'=>$price_min, 'price_max'=>$price_max, 'filter_attr'=>$temp_arrt_url, 'filter'=>$filter, 'year' => $year, 'output' => $output), $cat['cat_name']);
					...
					foreach ($attr_list as $k => $v)
					{
						...
						$all_attr_list[$key]['attr_list'][$temp_key]['url'] = build_uri('car_system_category', array('cid'=>$cat_id, 'bid'=>$brand, 'price_min'=>$price_min, 'price_max'=>$price_max, 'filter_attr'=>$temp_arrt_url, 'filter'=>$filter, 'year' => $year, 'output' => $output), $cat['cat_name']);
			...
			foreach ($filter_attr AS $fkey=>$fval)
			{
				if ($fval)
				{
					...
					$condition[]=array(
							...
							'cond_name' =>$cond_name,
							'cond_url' =>build_uri('car_system_category', array('cid'=>$cat_id, 'bid'=>$brand, 'price_min'=>$price_min, 'price_max'=>$price_max, 'filter_attr'=>$temp_arrt_url_68ecshop, 'filter'=>$filter, 'year' => $year, 'output' => $output), $cat['cat_name'])
						);
				}
			}
			$attr_group_more_txt = "";
		 ...
		 $filter_info = array(
			0=>array('id'=>0,'name'=>'全部','selected'=>0,'url'=>build_uri('car_system_category', array_merge($attr_url_value,array('filter'=>0, 'year' => $year, 'output' => $output)),'全部')),
			1=>array('id'=>1,'name'=>'网站自营','selected'=>0,'url'=>build_uri('car_system_category', array_merge($attr_url_value,array('filter'=>1, 'year' => $year, 'output' => $output)),'网站自营')),
			2=>array('id'=>2,'name'=>'入驻商店铺','selected'=>0,'url'=>build_uri('car_system_category', array_merge($attr_url_value,array('filter'=>2, 'year' => $year, 'output' => $output)),'入驻商店铺'))
		);
        ...
        if ($GLOBALS['_CFG']['rewrite']) {
            $year_url = build_uri('car_system_category', array_merge($attr_url_value, array('filter'=>$filter, 'output' => $output, 'year' => '#year#')));
            $output_url = build_uri('car_system_category', array_merge($attr_url_value, array('filter'=>$filter, 'year' => $year, 'output' => '#output#')));
        } else {
            $year_url = build_uri('car_system_category', array_merge($attr_url_value, array('filter'=>$filter, 'output' => $output)));
            $output_url = build_uri('car_system_category', array_merge($attr_url_value, array('filter'=>$filter, 'year' => $year)));
        }
        $smarty->assign('year_url', htmlspecialchars_decode($year_url));
        $smarty->assign('output_url', htmlspecialchars_decode($output_url));
        $smarty->assign('year_output_list', get_year_output());
		...
		$smarty->assign('yeared',         $year);
		$smarty->assign('outputed',         $output);
	...
	$smarty->assign('new_goods',       get_car_system_category_recommend_goods('new', $children, $brand, $price_min, $price_max, $ext));
		...
		assign_pager('car_system_category',            $cat_id, $count, $size, $sort, $order, $page, '', $brand, $price_min, $price_max, $display, $filter_attr_str,'','',$filter, $output, $year); // 分页
