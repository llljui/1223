	...
	$filter['suppliers_id'] = !empty($_REQUEST['suppliers_id']) ? (empty($_REQUEST['suppliers_id']) ? $_SESSION['suppliers_id'] : trim($_REQUEST['suppliers_id'])) : '';
        ...
        /* 记录总数 */
            ...
            if(!$real_goods && $filter['extension_code'] == 'virtual_good'){
                ...
                /*去除了店内分类，date 2017-04-21 wuzeguo*/
                $sql = "SELECT COUNT(distinct(g.goods_id)) FROM " .$GLOBALS['ecs']->table('goods'). " AS g,".$GLOBALS['ecs']->table('supplier_goods_cat')." as sgc WHERE is_delete='$is_delete' AND g.supplier_id='". $_SESSION['supplier_id'] ."' $where";
...
/* 代码增加_虚拟团购_START  bbs.hongyuvip.com */
         if(!$real_goods && $filter['extension_code'] == 'virtual_good'){
             ...
             $sql = "SELECT distinct(g.goods_id), goods_name, goods_type, goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot, sort_order,supplier_status, goods_number, integral, " .
                    " (promote_price > 0 AND promote_start_date <= '$today' AND promote_end_date >= '$today') AS is_promote ".
                    " FROM " . $GLOBALS['ecs']->table('goods') . " AS g,".$GLOBALS['ecs']->table('supplier_goods_cat')." as sgc WHERE is_delete='$is_delete' AND g.supplier_id='". $_SESSION['supplier_id'] ."' $where" .
                    " ORDER BY $filter[sort_by] $filter[sort_order] ".
                    " LIMIT " . $filter['start'] . ",$filter[page_size]";
         }
          /* 代码增加_虚拟团购_END  bbs.hongyuvip.com */
 ...
 * @author wuzeguo
 */
function format_csv2array($filePath = '', $sheet=0)
{
    require_once ROOT_PATH . '/plugins/Excel/PHPExcel/IOFactory.php';     //修改为自己的目录
    require_once ROOT_PATH . '/plugins/Excel/PHPExcel.php';     //修改为自己的目录
    if(empty($filePath) or !file_exists($filePath)){die('file not csv');}
    ...
    $currentSheet = $objPHPExcel->getSheet($sheet);        //**读取excel文件中的指定工作表*/
    $allColumn = $currentSheet->getHighestColumn();        //**取得最大的列号*/
    $highestColumnNum = PHPExcel_Cell::columnIndexFromString($allColumn) - 1;
    $allRow = $currentSheet->getHighestRow();        //**取得一共有多少行*/

    $data = array();
    for($rowIndex=1;$rowIndex<=$allRow;$rowIndex++){        //循环读取每个单元格的内容。注意行从1开始，列从A开始
        for($colIndex=0;$colIndex<=$highestColumnNum;$colIndex++){
            $colIndexE = PHPExcel_Cell::stringFromColumnIndex($colIndex);
            $addr = $colIndexE.$rowIndex;
            $cell = $currentSheet->getCell($addr)->getValue();
            if($cell instanceof PHPExcel_RichText){ //富文本转换字符串
                $cell = $cell->__toString();
            }
            $data[$rowIndex][$colIndexE] = $cell;
        }
    }
}
...
function format_excel2array($filePath='',$sheet=0){
    ...
    if ($column_num == 1) {
        ...
        $first_column = $allColumn[0];
        $second_column = $allColumn[1];

        //如果列号为两字母则执行
        for($rowIndex=4;$rowIndex<=$allRow;$rowIndex++){        //循环读取每个单元格的内容。注意行从1开始，列从A开始
            ...
            //22 两个字母的，但不包含最后一个
            //如果CL 则范围：AA-AZ BA-BA
            for ($colIndex='A'; $colIndex < $first_column; $colIndex++) {
                for($colIndex2='A';$colIndex2<='Z';$colIndex2++){
                    $addr = $colIndex.$colIndex2.$rowIndex;
                    $cell = $currentSheet->getCell($addr)->getValue();
                    if($cell instanceof PHPExcel_RichText){ //富文本转换字符串
                        $cell = $cell->__toString();
                    }
                    $data[$rowIndex][$colIndex.$colIndex2] = $cell;
                    if ($colIndex2 == 'Z') break;
                }
            }

            //CA-CL
            for($colIndex='A';$colIndex<=$second_column;$colIndex++){
                $addr = $first_column.$colIndex.$rowIndex;
                $cell = $currentSheet->getCell($addr)->getValue();
                if($cell instanceof PHPExcel_RichText){ //富文本转换字符串
                    $cell = $cell->__toString();
                }
                $data[$rowIndex][$first_column.$colIndex] = $cell;
            }
 ...
 * 导入商品时自动建立sku
 * 可能没有对应的价格，使用统一的价格
 * @param $attr_value_info array sku属性一唯数组
 * @param $goods_id int 商品id
 * @param $goods_info array 商品内容
 * @todo 如果存在相同名称的sku则自动取，不再重新生成
 * @author wuzeguo
 * @date 2017-04-27
 */
function autoBuildSku($attr_info, $attr_price_info, $goods_id, $goods_info)
{
    if (!$attr_info || !$goods_id || !$attr_price_info) return false;

    //获取最小价格
    $min_price = min($attr_price_info);

    $attr_string = '';
    foreach ($attr_info AS $attr_v) {
        $attr_string .=  $attr_v . '\n';
    }
    ...
    //1, goods_type
    $type_arr = array(
        'cat_name' => $goods_info['goods_name'],
        'enabled' => 1,
        'attr_group' => '淘宝商品属性分类'
    );
    $goods_type_id = $GLOBALS['db']->table('goods_type')->add($type_arr);
    ...
    if ($goods_type_id) {
        $attr_arr = array(
            ...
            'attr_input_type' => 1,
            'attr_type' => 1, 
            'attr_values' => $attr_string,
        );
        $attribute_id = $GLOBALS['db']->table('attribute')->add($attr_arr);
        ...
        if ($attribute_id) {
            ...
            $add_discount = 1+$add_discount/100;
            foreach ($attr_info AS $key => $value) {
                //如果sku价格大于最底价就取差价，反之为0
                $sku_price = $attr_price_info[$key];
                if (!$sku_price) continue;

                $diff_price = $sku_price - $min_price;
                $diff_price = $diff_price > 0 ? $diff_price : 0;
                $attr_arr = array(
                    'goods_id' => $goods_id,
                    'attr_id' => $attribute_id,
                    'attr_value' => $value,
                    'attr_price' => $diff_price * $add_discount,
                    'original_price' => $diff_price
                );
                $id = $GLOBALS['db']->table('goods_attr')->add($attr_arr);
                //4, products
                $rand_num = rand(10000, 15000);
                $total_num += $rand_num;
                $arr = array(
                    'goods_id' => $goods_id,
                    'goods_attr' => $id,
                    'product_number' => $rand_num,
                );
                $product_id = $GLOBALS['db']->table('products')->add($arr);
                $product_sn = $goods_info['goods_sn'] . 'g_p' . $product_id; 
                $GLOBALS['db']->table('products')->where('product_id = ' . $product_id)->setField('product_sn', $product_sn);
            }

            //修改商品总数量
            if ($total_num) {
                $GLOBALS['db']->table('goods')->where('goods_id = ' . $goods_id)->setField(array(
                    'goods_number' => $total_num,
                    'goods_type' => $goods_type_id,
                    'shop_price' => $min_price * $add_discount,
                    'market_price' => $min_price * ($add_discount+0.05),
