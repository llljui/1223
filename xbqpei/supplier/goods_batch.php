...
if ($_REQUEST['act'] == 'add')
{
    ...
    $smarty->assign('car_system_cat_list', car_system_cat_list());
    //品牌
    $brand_list = $GLOBALS['db']->table('brand')->where('is_show = 1')->select();
...
elseif ($_REQUEST['act'] == 'upload')
{
    ...
    //$data = format_csv2array($filePath);
    //dump($data);die;
    //end
    ...
    elseif($_POST['data_cat'] == 'taobao')
    {
        ...
        {
            // 跳过第一行
            /*if ($line_number == 0 || $line_number == 1 || $line_number == 2)
            {
                $line_number++;
                continue;
            }
            ...
            $arr['integral'] = 0;
            $arr['original_img'] = strip_tags(trim($line_list[28], '"'));
            $arr['keywords'] = '';
            $arr['goods_brief'] = '';
            $arr['goods_desc'] = strip_tags($line_list[50]);
            $arr['goods_desc'] = substr($arr['goods_desc'], 1, -1);
            $arr['goods_number'] = $line_list[9];
            $arr['warn_number'] =1;
            $arr['is_best'] = 0;
            $arr['is_new'] = 0;
            $arr['is_hot'] = 0;
            ...
            $arr['is_alone_sale'] = 0;
            $arr['is_real'] = 1;
            $arr['goods_img'] = strip_tags(trim($line_list[28], '"'));
            $arr['goods_thumb'] = strip_tags(trim($line_list[28], '"'));
            ...
            $arr    = array();
            $line_list = $line;
            $goods_name = strip_tags($line_list['A']);

            //如果数据库中存在相同的商品名称则过滤掉 
            $have_goods = $db->table('goods')->where('is_delete = 0 AND goods_name = "' . $goods_name . '"')->find();
            if ($have_goods) continue;
            $arr['goods_name'] = $goods_name;

            $max_id     = $db->getOne("SELECT MAX(goods_id) FROM ".$ecs->table('goods'));
            $max_id = intval($max_id) + $id_is;
            $id_is++;
            $goods_sn   = generate_goods_sn($max_id);
            $arr['goods_sn'] = $goods_sn;
            //$arr['brand_name'] = '';
            $add_discount = $GLOBALS['_CFG']['add_discount'];
            $add_discount || $add_discount = 0;
            $market_add_discount = $GLOBALS['_CFG']['market_add_discount']; //市场提高价
            $market_add_discount || $market_add_discount = 5;
            $arr['market_price'] = floatval($line_list['H']) * (1+$add_discount/100+$market_add_discount/100); //原价上加配置项+5%
            $arr['shop_price'] = floatval($line_list['H']) * (1+$add_discount/100);
            //$arr['integral'] = 0;
            $arr['original_img'] = strip_tags(trim($line_list['AC'], '"'));
            //$arr['keywords'] = '';
            //$arr['goods_brief'] = '';
            //$arr['goods_desc'] = htmlspecialchars($line_list['U']);
            $arr['goods_number'] = intval($line_list['J']);
            //$arr['warn_number'] =1;
            //$arr['is_best'] = 0;
            //$arr['is_new'] = 0;
            //$arr['is_hot'] = 0;
            ...
            $arr['is_alone_sale'] = 1;
            $arr['is_real'] = 1;
            $arr['goods_img'] = strip_tags(trim($line_list['AC'], '"'));
            $arr['goods_thumb'] = strip_tags(trim($line_list['AC'], '"'));
            //商品原地址
            $arr['original_url'] = $_LANG['goods_original_url']['taobao'] . strip_tags($line_list['AH']);
            $arr['original_price'] = floatval($line_list['H']); //原价

            //商品属性处理
            $arr['goods_attr'] = $line_list['BD'];
            $arr['goods_attr_price'] = $line_list['AE'];

            $goods_list[] = $arr;

            //详情放入数据库
            $goods_desc = htmlspecialchars($line_list['U']);
            $temp_ids[] = $db->table('goods_desc_temp')->add(array('content' =>json_encode($goods_desc)));
        }
    }
    ...
    $temp_ids = implode(',', $temp_ids);
    $smarty->assign('temp_ids', $temp_ids);
    ...
    // 显示的字段列表
    $smarty->assign('field_show', array('goods_name' => true, 'goods_sn' => true, 'market_price' => true, 'shop_price' => true));
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    if (isset($_POST['checked']))
    {
        ...
        //处理商品详情页
        $temp_ids = explode(',', $_POST['temp_ids']);
        ...
        foreach ($_POST['checked'] AS $key => $value)
        {
            ...
            $field_arr = array(
                'cat_id'        => $_POST['cat_id'],
                'system_cat_id' => $_POST['system_cat_id'],
                'original_url' => $_POST['original_url'][$value],
                'original_price' => $_POST['original_price'][$value],
                'add_time'      => gmtime(),
                'last_update'   => gmtime(),
            ...
            foreach ($field_list AS $field)
            {
                ...
                //商品详情处理
                if ($field == 'goods_desc') {
                    $temp_id = $temp_ids[$value];
                    $content = $db->table('goods_desc_temp')->where('goods_desc_temp_id = ' . $temp_id)->getField('content');   
                    $content = htmlspecialchars_decode(json_decode($content));
                    $field_arr[$field] = $content;
                }

                //把值付给goods_img_back
                $goods_img_back = $field_arr['original_img'];

                // 特殊处理
                if (!empty($field_value))
                {
                    // 图片路径
                    if (in_array($field, array('original_img', 'goods_thumb', 'goods_img')))
                    {

                        if(strpos($field_value,'|;')>0)
                        {
                            $image_info = array();
                            $image_list = explode('|;', $field_value);
                            foreach ($image_list AS $images) {
                                $field_value=explode(':',$images);
                                $field_value=$field_value['0'];
                                @copy(ROOT_PATH.'taobaopic/'.$field_value.'.tbi',ROOT_PATH.'images/taobao/'.$field_value.'.jpg');
                                if(is_file(ROOT_PATH.'images/taobao/'.$field_value.'.jpg'))
                                {
                                    $image_info[] = 'images/taobao/' . $field_value.'.jpg';
                                }
                            }
                            $field_arr[$field] = $image_info[0];

                            //各自要存不同地方 
                            if ($field == 'original_img') {
                                $field_arr['original_img_info'] = $image_info;
                            } elseif ($field == 'goods_img') {
                                $field_arr['goods_img_info'] = $image_info;
                            } elseif ($field == 'goods_thumb') {
                                $field_arr['goods_thumb_info'] = $image_info;
                            }
                        }
                        else
                        {
                            $field_arr[$field] = ROOT_PATH . 'images/' . $field_value;
                        }
                      }
                    // 品牌
            ...
            //在原来的价格上提价10%
            $add_discount = $GLOBALS['_CFG']['add_discount'] > 0 ? $GLOBALS['_CFG']['add_discount'] : 10;
            $field_arr['add_discount'] = $add_discount;

            //所属添加者supplier
            //added by wuzeguo
            $supplier_id = isset($_SESSION['supplier_id']) ? intval($_SESSION['supplier_id']) : $_COOKIE['ECSCP']['supplier_id'];
            $field_arr['supplier_id'] = $supplier_id;
            $field_arr['supplier_status'] = 0;
            //dump($field_arr);die;
            ...
            if (!empty($field_arr['original_img']) || !empty($field_arr['goods_img']) || !empty($field_arr['goods_thumb']))
            {
                ...
                if (!empty($field_arr['original_img']))
                {
                    ...
                    if ($_CFG['auto_generate_gallery'])
                    {
                        ...
                        //直接从原地把图片放到指定地方，不删除原图
                        $original_info = array();
                        foreach ($field_arr['original_img_info'] AS $img) {
                            //$gallery_img = dirname($img) . '/' . $image->random_filename() . $ext;
                            ...
                            //先压缩
                            $original_info[] = reformat_image_name('gallery', $goods_gallery['goods_id'], $img, 'source');
                        }
                        $goods_gallery['img_original'] = $original_info;
                    }
                    //设置商品原图
                    if ($_CFG['retain_original_img'])
                    {
                        //$original_img                  = reformat_image_name('goods', $goods_gallery['goods_id'], $field_arr['original_img'], 'source');
                        $original_img = $original_info[0];
                    }
                    else
                    {
                ...
                if (!empty($field_arr['goods_img']))
                {
                    ...
                    //同理使用
                    $goods_img_info = array();
                    foreach ($field_arr['goods_img_info'] AS $img) {
                        $img_url = $image->make_thumb(ROOT_PATH . $img, $GLOBALS['_CFG']['image_width'],  $GLOBALS['_CFG']['image_height']);
                        $goods_img_info[] = reformat_image_name('goods', $goods_gallery['goods_id'], $img_url, 'goods');
                        @unlink(ROOT_PATH . $img_url);
                    }
                    $goods_gallery['img_url'] = $goods_img_info;
                    $goods_img                = $goods_img_info[0];
                ...
                if (!empty($field_arr['goods_thumb']))
                {
                    ...
                    $goods_thumb_info = array();
                    foreach ($field_arr['goods_thumb_info'] AS $img) {
                        $thumb_url = $image->make_thumb(ROOT_PATH . $img, $GLOBALS['_CFG']['thumb_width'],  $GLOBALS['_CFG']['thumb_height']);
                        $goods_thumb_info[] = reformat_image_name('goods_thumb', $goods_gallery['goods_id'], $thumb_url, 'thumb');
                        @unlink(ROOT_PATH . $thumb_url);
                    }
                    $goods_gallery['thumb_url'] = $goods_thumb_info;
                    $goods_thumb = $goods_thumb_info[0];
                }
                ...
                if ($_CFG['auto_generate_gallery'])
                {
                    foreach ($goods_gallery['img_original'] AS $k => $v) {

                        $gallery_info = array();
                        $gallery_info['goods_id'] = $goods_gallery['goods_id'];
                        $gallery_info['img_original'] = $v;
                        $gallery_info['img_url'] = $goods_gallery['img_url'][$k];
                        $gallery_info['thumb_url'] = $goods_gallery['thumb_url'][$k];

                        $db->autoExecute($ecs->table('goods_gallery'), $gallery_info, 'INSERT');
                    }
                }
            }
            ...
            //商品默认为1627207
            $type = '1627207';
            if ($_POST['goods_attr'][$value] && $_POST['goods_attr_price'][$value]) {
                //属性价格 此处暂时只能解析两种情况，如遇其他情况再解析
                $attr_price_list = explode(';', $_POST['goods_attr_price'][$value]); 
                $attr_price_info = array();

                if (strpos($attr_price_list[0], $type)) {
                    //格式：单属性
                    //价格:621:500::1627207:-1001;
                    //属性:1627207:-1001:320LI 前盘一只;
                    foreach ($attr_price_list AS $ke => $va) {
                        $attr_price_1 = explode('::', $va);
                        $price = explode(':', $attr_price_1[0])[0];
                        $price_name = explode(':', $attr_price_1[1]);
                        if ($price_name[0] == $type) $attr_price_info[$price_name[1]] = $price;
                    }
                } elseif (strpos($attr_price_list[0], $type) == false && strpos($attr_price_list[1], $type) == 0) {
                    //格式：两属性
                    //价格:48:4653::21433:10122;1627207:-1013;
                    //属性:1627207:-1001:支持99％以上车型/拍此选项请留;
                    foreach ($attr_price_list AS $ke => $va) {
                        if ($ke % 2 == 1) {
                            $price_name = explode(':', $va);
                            $price = explode(':', $attr_price_list[$ke - 1])[0];
                            $attr_price_info[$price_name[1]] = $price;
                        }
                    }
                }

                //属性名称
                $attr_list = explode(';', $_POST['goods_attr'][$value]);
                $attr_info = array();
                foreach ($attr_list AS $key => $attr_v) {
                    $attr_va = explode(':', $attr_v);
                    if ($attr_va[0] == '1627207') {
                        $attr_info[$attr_va[1]] = $attr_va[2];
                    }
                }

                //supplier/includes/lib_goods.php
                autoBuildSku($attr_info, $attr_price_info, $goods_id, $field_arr);
                if (count($attr_price_info) == count($attr_info)) {
                }
            }
        }
    }
