...
function dobid2_ok($deal_id,$user_id){
    ...
    //投标后检测是否到了累计金额，并返红包
    check_deal_load_money();

}

//检测累计金额
//返回成功或失败
function check_deal_load_money()
{
    $user_id = $GLOBALS['user_info']['id'];   
    if (!$user_id) return false;

    //是否开启
    if (app_conf('INVEST_BACK_STATUS') != 1) return false;

    //统计投资总额, 在投资时间内
    $cate_ids = app_conf('INVEST_CATE');
    $where = 'dl.user_id = ' . $user_id . ' AND d.cate_id IN (' . $cate_ids . ')';
    $start_time = app_conf('INVEST_START_TIME');
    $end_time = app_conf('INVEST_END_TIME');
    if ($start_time) $where .= ' AND dl.create_time >= ' . $start_time;
    if ($end_time) $where .= ' AND dl.create_time <= ' . $end_time;
    logger::write('where:' . $where);

    $sql = 'select dl.money, dl.deal_id from ' . DB_PREFIX . 'deal_load as dl LEFT JOIN ' . DB_PREFIX . 'deal as d ON dl.deal_id = d.id where ' . $where;
    $list = $GLOBALS['db']->getAll($sql);

    $total_money = 0;
    foreach ($list AS $k=>$v) {
        $total_money += intval($v['money']);
    }
    logger::write('total_money:' . $total_money);

    //找出所有配置信息相关信息
    //现有以下几种累计返利:50000(5), 30000(4), 10000(3), 5000(2), 1000(1)
    #if ($total_money >= 1000) put_ecv(1, $user_id);
    #if ($total_money >= 5000) put_ecv(2, $user_id);
    #if ($total_money >= 10000) put_ecv(3, $user_id);
    #if ($total_money >= 30000) put_ecv(4, $user_id);
    #if ($total_money >= 50000) put_ecv(5, $user_id);
    //返现金
    if ($total_money >= 1000) put_left_amount(1, $user_id);
    if ($total_money >= 5000) put_left_amount(2, $user_id);
    if ($total_money >= 10000) put_left_amount(3, $user_id);
    if ($total_money >= 30000) put_left_amount(4, $user_id);
    if ($total_money >= 50000) put_left_amount(5, $user_id);
    #dump($set_info);die;
}

//现将以前的返红包改为返现金到余额
//@author wuzeguo
function put_left_amount($type=0, $user_id=0)
{
    if (!$type) return;
    ...
    #$ecv_type_id = $GLOBALS['db']->table('ecv_type')->where('type='. $type)->getField('id');
    #if (!$ecv_type_id) return;
    //查看此红包是否已发送给用户
    #$have = $GLOBALS['db']->table('ecv')->where('user_id = ' . $user_id . ' AND ecv_type_id = ' . $ecv_type_id)->find();
    #if ($have) return;

    //查看是否已发放过此现金
    $have = $GLOBALS['db']->table('user_money_log')->where('from_amount_type = ' . $type . ' AND user_id = ' . $user_id)->find();
    if ($have) return;

    require_once APP_ROOT_PATH."system/libs/user.php";
    $amount = $money[$type];
    $msg1 = $msg[$type];
    modify_account(array('money'=>$amount, 'from_amount_type' => $type),$user_id,$msg1,24);
...
//已经发过的红包不再发放
//param $type 
function put_ecv($type, $user_id=0)
{
    $ecv_type_id = $GLOBALS['db']->table('ecv_type')->where('type='. $type)->getField('id');
    if (!$ecv_type_id) return;
    ...
    include_once APP_ROOT_PATH . 'system/libs/voucher.php';
    
    $id = send_voucher($ecv_type_id, $user_id);
...
function dobid2($deal_id,$bid_money,$bid_paypassword,$is_pc=0,$ecv_id=0,$learn_id=0,$interestrate_id=0,$use_interestrate=0){
    ...
    //投资成功之前总投资额
    $have_bid_money_list = $GLOBALS['db']->table('deal_load')->where('user_id = ' . $GLOBALS['user_info']['id'])->getField('money', true);
    $have_bid_money = $bid_money;
    foreach ($have_bid_money_list AS $k=>$v) {
        $have_bid_money += $v;
    }
	...
	if($load_id > 0){
        ...
        $user_id = $GLOBALS['user_info']['id'];
        if ($pid) {
            if ($have_bid_money >= 100) {
                //查看是否已民发放
                $is_put = $GLOBALS['db']->table('user_money_log')->where('type = 60 AND user_id = ' . $pid . ' AND from_user_id = ' . $user_id)->find();
                if (!$is_put) {
                    require_once APP_ROOT_PATH."system/libs/user.php";
                    $msg1 = '邀请用户' . $GLOBALS['user_info']['user_name'] . '(投资100元)返现金'; 
                    modify_account(array('money'=>10, 'from_user_id' => $user_id),$pid,$msg1,60);
                }
            }
        }

        //邀请投资返利
        get_referral_by_deal($load_id);
