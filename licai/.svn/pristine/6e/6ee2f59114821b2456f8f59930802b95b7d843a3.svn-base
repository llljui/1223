 ...
 * wuzeguo
 */
function parse_script_debug($urls, $encode_url=array())
{
    $js = '';
    if (IS_DEBUG) {
        foreach ($urls as $url) {
        ...
        $url = parse_script($urls, $encode_url);
        $js = '<script src="'.$url.'"></script>';
    }
...
function site_statics(){
	...
	if($stats==false){
		...
		$v_load_count  = $GLOBALS['db']->getOne("SELECT value FROM ".DB_PREFIX."conf where  name= 'VIRTUAL_DEAL_COUNT'");
		$stats['load_count'] = $load_rs['load_count'] + $v_load_count;
		...
		$nums = $GLOBALS['db']->getOne("SELECT value FROM ".DB_PREFIX."conf where  name= 'VIRTUAL_DEAL_FINISH_NUM'");
        $stats['deal_total_count'] += $nums;
        ...
        $stats['user_count_format'] = $stats['user_count'] + $GLOBALS['config']['VIRTUAL_USER'];
