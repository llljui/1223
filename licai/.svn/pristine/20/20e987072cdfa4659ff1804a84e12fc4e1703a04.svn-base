...
class ConfAction extends CommonAction{
    ...
    //设置投资返现
    //added by wuzeguo
    public function set_invest_back()
    {
		#$conf_res = M("Conf")->where("is_effect = 1 and `name` in ('INVEST_BACK_STATUS', 'INVEST_1000_MONEY','INVEST_5000_MONEY', 'INVEST_10000_MONEY', 'INVEST_30000_MONEY', 'INVEST_50000_MONEY')")->order("group_id asc,sort asc")->findAll();
		$conf_res = M("Conf")->where("is_effect = 1 and `name` in ('INVEST_BACK_STATUS', 'INVEST_START_TIME','INVEST_END_TIME', 'INVEST_CATE')")->order("group_id asc,sort asc")->findAll();
		...
		$this->assign("conf",$conf);
		$this->assign("main_title","设置投资返现");
        $this->display();
    }

    public function update_invest_back()
    {
		$conf_res = M("Conf")->where("is_effect = 1 and `name` in ('INVEST_BACK_STATUS', 'INVEST_START_TIME','INVEST_END_TIME', 'INVEST_CATE')")->order("group_id asc,sort asc")->findAll();
		foreach($conf_res as $k=>$v)
		{
			if($v['name']=='INVEST_START_TIME' || $v['name']=='INVEST_END_TIME'){
				conf($v['name'],strtotime($_REQUEST[$v['name']]));
			}
			else{
				conf($v['name'],strim($_REQUEST[$v['name']]));
			}
		}
