...
class smtp
{
    ...
    function smtp($relay_host = "", $smtp_port = 25,$auth = FALSE,$user,$pass)
    {
        ...
        if ($smtp_item) {
            if ($smtp_item['is_ssl']) {
                $this->relay_host = 'ssl://' . $smtp_item['smtp_server'];
            } else {
                $this->relay_host = $smtp_item['smtp_server'];
            }
	    	$this->smtp_port = $smtp_item['smtp_port'];
	    	$this->user = $smtp_item['smtp_name'];
	    	$this->pass = $smtp_item['smtp_pwd'];
	    	$this->auth = $smtp_item['is_verify'];
        } else {
        ...
        $this->log_file = dirname(__FILE__) . "/../../smtp_log.log";
        $this->sock = FALSE;
