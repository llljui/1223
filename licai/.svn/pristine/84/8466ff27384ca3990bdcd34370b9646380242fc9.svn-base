...
//		$user['vip_state'] = 1;
	 ...
	 * 					30:配资本金(冻结); 31:配资预交款(冻结);32:配资审核费(冻结);33:配资服务费(平台收入);34:配资利息(出资者收入);35:配资平仓收益;36:配资投资;37:配资提取赢余;38:配资交易佣金;47:体验金收益;50邀请并投资返现
	 */
	function modify_account($data,$user_id,$log_msg='',$type=0)
	{
		...
		if (isset($data['money'])){
            ...
            if ($data['from_user_id']) {
                $money_log_info['from_user_id'] = $data['from_user_id'];
            }
            //累计投资返现标识
            if ($data['from_amount_type']) {
                $money_log_info['from_amount_type'] = $data['from_amount_type'];
            }
