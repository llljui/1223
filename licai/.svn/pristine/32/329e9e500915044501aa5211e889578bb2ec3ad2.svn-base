...
class userModule extends SiteBaseModule
{
	...
	public function doregister()
	{
		...
		if($res['status'] == 1)
		{
            ...
            $return_obj = syn_data_to_qipei($data, 'syn_register_user');
            $response_code = $return_obj->response_code;
            $xbqp_user_id = intval($return_obj->xbqp_user_id);
            if ($response_code == 1 && $xbqp_user_id) {
                //同步id
	...
	public function dologin()
	{
        ...
        logger::write('pwd:' . $_POST['user_pwd']);
