...
class uc_accountModule extends SiteBaseModule
{
	...
	public function saveemail(){
        ...
        //同步邮箱
        if ($email) {
            $xbqp_user_id = $GLOBALS['db']->table('user')->where('id = ' . $GLOBALS['user_info']['id'])->getField('xbqp_user_id');
            $data = array(
                'xbqp_user_id' => $xbqp_user_id,
                'email' => $email
            );
            syn_data_to_qipei($data, 'syn_bind_email');
        }
	...
	public function save()
	{
		...
		if($res['status'] == 1)
		{
            ...
            //同步数据
            //修改密码
            if ($_REQUEST['user_pwd'] && $_REQUEST['old_password']) {
                $data = array(
                    'xbqp_user_id' => $user_info['xbqp_user_id'],
                    'password' => $_REQUEST['user_pwd']
                );
                syn_data_to_qipei($data, 'syn_edit_pass');
            }
