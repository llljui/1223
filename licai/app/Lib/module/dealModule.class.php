...
class dealModule extends SiteBaseModule
{
	public function index(){
        ...
        //常见问题
        $cat_id = $GLOBALS['db']->table('article_cate')->where('title LIKE "常见问题%"')->getField('id');
        $question_list = $GLOBALS['db']->table('article')->where('is_delete = 0 AND is_effect = 1 AND cate_id=' . $cat_id)->order('sort')->select();
		$GLOBALS['tmpl']->assign("question_list",$question_list);
