    ...
    function credit_save(){
        ...
        if($this->is_ajax==1) {
            ...
            if($type == "credit_identificationscanning"){
                ...
                $xbqp_user_id = $GLOBALS['db']->table('user')->where('id = ' . $s_user_info['id'])->getField('xbqp_user_id');
                $data = array(
                    'real_name' => $_REQUEST['real_name'],
                    'card' => $_REQUEST['idno'],
                    'xbqp_user_id' => $xbqp_user_id,
                    'sex' => intval($_REQUEST['sex']),
                    'birthday' => intval($_REQUEST['byear']) . '-' .intval($_REQUEST['bmonth']) .'-' .  intval($_REQUEST['bday']),
                );
                if ($file_list) {
                    $data['face_card'] = 'http://' . $GLOBALS['_SERVER']['HTTP_HOST'] . $file_list[0];
                    $data['back_card'] = 'http://' . $GLOBALS['_SERVER']['HTTP_HOST'] . $file_list[1];
                }
                syn_data_to_qipei($data, 'syn_submit_real_info');
            }
