...
//手机绑定
    ...
    function fuyouquai_pay()
    {
        ...
        $payment_info = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."payment where class_name = 'Fuyouquai'");
        require_once APP_ROOT_PATH."app/Lib/uc_func.php";
		$status = getInchargeDone($payment_info['id'],$money,$bank_id,$memo,$pingzheng);

        require_once APP_ROOT_PATH."system/payment/Fuyouquai_payment.php";
        $payment_object = new Fuyouquai_payment();
        $payment_code = $payment_object->get_payment_code($status['order_id']);
