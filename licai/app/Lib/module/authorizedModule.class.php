...
LEFT JOIN ".DB_PREFIX."user t_u ON t_u.id = t_user_id LEFT JOIN ".DB_PREFIX."user f_u ON f_u.id = dl.user_id where  (f_u.pid='".$authorized_info["id"]."' or t_u.pid = '".$authorized_info["id"]."') and ((f_u.id = '".$user_id."' and (t_u.id is null or t_u.id = 0)) or t_u.id = '".$user_id."') group by dl.id order by dl.create_time desc ".$limit;
...
LEFT JOIN ".DB_PREFIX."user t_u ON t_u.id = t_user_id LEFT JOIN ".DB_PREFIX."user f_u ON f_u.id = dl.user_id where d.is_delete=0 and d.publish_wait = 0  and (f_u.pid='".$authorized_info["id"]."' or t_u.pid = '".$authorized_info["id"]."') and ((f_u.id = '".$user_id."' and (t_u.id is null or t_u.id = 0)) or t_u.id = '".$user_id."')";
...
LEFT JOIN ".DB_PREFIX."user t_u ON t_u.id = t_user_id LEFT JOIN ".DB_PREFIX."user f_u ON f_u.id = dl.user_id where  (f_u.pid='".$authorized_info["id"]."' or t_u.pid = '".$authorized_info["id"]."') group by dl.id order by dl.create_time desc ".$limit;
			
			$sql_count = "select count(DISTINCT dl.id) from ".DB_PREFIX."deal d left join ".DB_PREFIX."deal_load as dl on d.id = dl.deal_id LEFT JOIN ".DB_PREFIX."deal_load_transfer dlt on dlt.load_id = dl.id LEFT JOIN ".DB_PREFIX."user u ON u.id=d.user_id 
LEFT JOIN ".DB_PREFIX."user t_u ON t_u.id = t_user_id LEFT JOIN ".DB_PREFIX."user f_u ON f_u.id = dl.user_id where d.is_delete=0 and d.publish_wait = 0 and (f_u.pid='".$authorized_info["id"]."' or t_u.pid = '".$authorized_info["id"]."')";
		}
		...
		if($count >0){
			...
			foreach($list as $k=>$v){
				...
				switch($v['deal_status']){
					case "1":$list[$k]['status_format'] = '进行中' ; break;
					case "2":$list[$k]['status_format'] = '满标'; break;
					case "3":$list[$k]['status_format'] = "流标"; break;
