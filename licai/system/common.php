...
{
    logger::write('mobile:' . $mobile);
    logger::write('code:' . $code);
	$re = array('msg_id'=>0, 'status'=>0,'msg'=>'');
	
	if(app_conf("SMS_ON")==1)
	{
    logger::write('sms_on:1');
				...
				if ($immediately && $msg_id > 0){
                    $msg_data['data'] = "{\"code\":\"$code\",         \"product\":\"注册\"}";
					$result = send_sms_email($msg_data);
...
function send_sms_email($msg_item){
	...
	if($msg_item['send_type']==0)
	{
		...
		$sms = new sms_sender();
        $content = array($msg_item['content'], $msg_item['data']);
		$result = $sms->sendSms($msg_item['dest'],$content);
		//发送结束，更新当前消息状态
    ...
    {
        //初始化
        require_once APP_ROOT_PATH."system/utils/emailer.php";
        $smtp = new smtp();
        //关闭调试
        $smtp->debug = false;
        //测试发送
        if(!$smtp->smtp_sockopen()){		  
            logger::write('email error');
        }
        //以TXT形式发送 tesr@qq.com 为收信EMAIL
        $result = $smtp->sendmail($msg_item['dest'], $GLOBALS['config']['SHOP_TITLE'],'', $msg_item['title'], $msg_item['content'],'TXT');

        logger::write('content:' . $msg_item['content']);
 ...
 * $type 0 正常借款 1众筹
 */
function get_referral_by_deal($deal_load_id,$type = 0){
    //是否开启返利
    $is_open = app_conf('INVITE_REFERRALS');
    if (!$is_open) return;

    logger::write('start referral');
	if($type == 0)
	{
        $deal_load_info = $GLOBALS['db']->table('deal_load')->where('id = ' . $deal_load_id)->find();
        $user_info = $GLOBALS['db']->table('user')->where('id = ' . $deal_load_info['user_id'])->find();
    logger::write('referral user_info:'. json_encode($user_info));
        if ($user_info['pid']) {

            //注册之日起几个月
            $end_time = strtotime('+1 month', $user_info['create_time']);

            if ($deal_load_info['create_time'] >= $user_info['create_time'] && $deal_load_info['create_time'] <= $end_time) {

                    $rate = 0.2;
                    $gain_money = $deal_load_info['money'] * $rate / 100;
    logger::write('referral gain:' . $gain_money);
                    //发放
                    if ($gain_money) {
                        $data['deal_id'] = $deal_load_info['deal_id'];
                        $data['load_id'] = $deal_load_id;
                        $data['l_key'] = 0;
                        $data['money'] = $gain_money;
                        $data['user_id'] = $user_info['id'];
                        $data['rel_user_id'] = $user_info['pid'];
                        $data['referral_type'] = 0;
                        $data['referral_rate'] = $rate;
                        $data['repay_time'] = 0;
                        $data['create_time'] = TIME_UTC;
                        $GLOBALS['db']->autoExecute(DB_PREFIX."referrals",$data,"INSERT");
                        $id = $GLOBALS['db']->insert_id();
    logger::write('referral id:' . $id);
                        if($id > 0)
                        {
                            //开始发放返利
                            require_once APP_ROOT_PATH."system/libs/user.php";
                            ...
                            $msg = '[<a href="'.$deal['url'].'" target="_blank">'.$deal['name'].'</a>],获取邀请返利';
                            modify_account(array('money'=>round($data['money'],2)),$user_info['pid'],$msg,23);
                            $GLOBALS['db']->query("UPDATE ".DB_PREFIX."referrals  SET pay_time=".TIME_UTC." WHERE id=".$id);
...
function checkVeifyCode($code,$vname="verify"){
    ...
    $md5_code = es_session::get($vname);
    logger::write('md5_code:' . $md5_code);
    logger::write('md5_code:' . md5($code));
    if ($md5_code == md5($code)) {
        return true;
    }
    return false;
}
 ...
 * @return array
 */
function syn_data_to_qipei($data, $func)
{
    require_once (dirname(__FILE__) . '/utils/es_syn_user.php');
    $syn_obj = new es_syn_user();
    return $syn_obj->$func($data);
}
